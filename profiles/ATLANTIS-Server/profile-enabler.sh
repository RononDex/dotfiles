...
sudo cp -r $scriptDir/overrides/scripts /root/scripts
sudo cp ~/.scripts/backup/backupResticServer /root/backupResticServer
...
if [ ! -d /etc/sv/dyndns-updater/ ]; then
	...
	sudo chmod +x /etc/sv/dyndns-updater/run
