...
chmod -R +x ~/.scripts
chmod +x ~/.config/aerc/autocomplete.sh
chmod 700 ~/.gnupg -R
...
sudo pacman -S pkgconfig man-db wget bc powerline-fonts debugedit ttf-firacode-nerd ttf-arimo-nerd gcc boost ffmpeg make cmake otf-fira-mono otf-fira-sans ttf-fira-code ttf-fira-mono ttf-fira-sans zsh zsh-completions automake m4 autoconf --noconfirm --needed
...
sudo pacman -S zsh-syntax-highlighting openvpn zsh-autosuggestions starship calc diff-so-fancy networkmanager-openvpn powerline-fonts zathura zathura-cb zathura-pdf-mupdf zathura-ps lynx ttf-dejavu --needed --noconfirm
