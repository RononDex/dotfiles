...
sudo xbps-install -y htop kitty chafa ImageMagick zlib xdg-utils curl exfat-utils unzip shadow perl-AnyEvent-I3 perl-JSON-XS git-lfs fzf pass patch ncurses ncurses-devel
...
sudo xbps-install -y tar rust cargo zip xz glibc-32bit keyutils bc elfutils-devel flex gmp-devel kmod libmpc-devel openssl-devel perl uboot-mkimage cpio pahole python3 vips lm_sensors chroma thefuck
